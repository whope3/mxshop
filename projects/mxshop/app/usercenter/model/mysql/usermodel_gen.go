...
type (
	...
	User struct {
		...
		Gender   int64     `db:"gender"`   // gender,1 male|2 female|0 unknown
...
func (m *defaultUserModel) Insert(ctx context.Context, data *User) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.Uid, data.Name, data.Password, data.Mobile, data.Gender)
	return ret, err
}

func (m *defaultUserModel) Update(ctx context.Context, newData *User) error {
	query := fmt.Sprintf("update %s set %s where `id` = ?", m.table, userRowsWithPlaceHolder)
	_, err := m.conn.ExecCtx(ctx, query, newData.Uid, newData.Name, newData.Password, newData.Mobile, newData.Gender, newData.Id)
	return err
}
